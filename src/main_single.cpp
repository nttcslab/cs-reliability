...
int main(int argc, char **argv){
  ...
  size_t dst_final = 0;
  {
    ...
    for(size_t i=0; i<m; ++i){
      ...
      // dest find
      if(G.e[i].first == dest || G.e[i].second == dest){
        dst_final = i;
      }
    }
  ...
  for(size_t ind=snum-1; ind>=2; --ind){
    DPBlock& dp_now = dp[ind];
    if(dp_now.level < dst_final) break;
  ...
  {
    size_t k_pos = 0;
    size_t kk = fro[dst_final].size();
    for(size_t k=0; k<kk; ++k){
      if(dest == fro[dst_final][k]){
        k_pos = k;
        break;
      }
    }
    
    for(const auto& ent : maps[dst_final]){
      size_t now_id = ent.second;
      const State& now_state = ent.first;
      
      res += dp[now_id].p * dp[now_id].q[now_state.second[k_pos]];
    }
  }
  printf("%d : %.15lf\n", dest, res);
